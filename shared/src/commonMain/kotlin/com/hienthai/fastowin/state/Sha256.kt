...
private val sha256Constants = intArrayOf(
...
internal fun sha256(input: ByteArray): String {
    ...
    val data = ByteArray(input.size + 1 + padding + 8)
    input.copyInto(data)
    data[input.size] = 0x80.toByte()
    for (index in 0 until 8) {
        data[data.lastIndex - index] = (bitLength ushr (index * 8)).toByte()
    }
    ...
    for (chunkStart in data.indices step 64) {
        ...
        for (index in 0 until 64) {
            ...
            val temp1 = h + sum1 + choice + sha256Constants[index] + words[index]
