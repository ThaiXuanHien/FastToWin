...
Trên Web, refresh token không được trả trong JSON hoặc lưu trong `localStorage`. Server đặt token trong cookie host-only `HttpOnly`, `SameSite=Strict`; production dùng thêm `Secure` và tiền tố `__Host-`. Mọi yêu cầu sử dụng cookie đều phải có `Origin` thuộc `FASTTOWIN_WEB_ORIGINS` cùng hai header xác nhận phiên Web/CSRF. Access token chỉ tồn tại trong bộ nhớ của tab và được cấp lại từ cookie sau khi tải lại trang. Android vẫn dùng Android Keystore và iOS dùng Keychain như trước.
