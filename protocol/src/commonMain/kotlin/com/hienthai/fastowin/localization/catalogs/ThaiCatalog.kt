...
) + thaiShellAuthTexts + thaiGameplayTexts + thaiProfileTexts + thaiSocialShopTexts + thaiProtocolMessageTexts + thaiDeliveryTexts + residualTexts(AppLanguage.THAI) + economyProgressionTexts.getValue(AppLanguage.THAI) + playQuotaTexts.getValue(AppLanguage.THAI),
    quantities = mapOf(
        QuantityKey.Players to mapOf(
            PluralCategory.OTHER to "ผู้เล่น {count} คน"
        )
    ) + thaiProfileQuantities + thaiSocialShopQuantities
)
